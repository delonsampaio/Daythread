...
extension Trip {
    ...
    var daysArray: [TripDay] {
        (days as? Set<TripDay>)?.filter { $0.isAlive }
            .sorted { $0.sortOrder < $1.sortOrder } ?? []
    }
    var membersArray: [TripMember] {
        (members as? Set<TripMember>)?.filter { $0.isAlive }
            .sorted { $0.joinedAt < $1.joinedAt } ?? []
    }
    var documentsArray: [TripDocument] {
        (documents as? Set<TripDocument>)?.filter { $0.isAlive }
            .sorted { $0.addedAt < $1.addedAt } ?? []
    }
    var expensesArray: [TripExpense] {
        (expenses as? Set<TripExpense>)?.filter { $0.isAlive }
            .sorted { $0.date < $1.date } ?? []
    }
    var preTripTasksArray: [PreTripTask] {
        (preTripTasks as? Set<PreTripTask>)?.filter { $0.isAlive }
            .sorted { $0.sortOrder < $1.sortOrder } ?? []
    }
    var lodgingArray: [LodgingInfo] {
        (lodging as? Set<LodgingInfo>)?.filter { $0.isAlive }
